// app/api/v1/questions/[id]/versions/[versionNumber]/rollback/route.ts
// Rollback question to a previous version

import { NextRequest } from 'next/server';
import { auth } from '@/auth';
import { apiHandler, apiSuccess, ApiError, unauthorized, forbidden, notFound } from '@/lib/api-handler';
import { rollbackQuestion } from '@/lib/version-control';
...
const handler = apiHandler(async (
  ...
  // Verify question exists and get author
  ...
  const isAuthor = question.author_id === session.user.id;
  const isAdmin = user?.role === 'ADMIN';

  if (!isAuthor && !isAdmin) {
    throw forbidden('Only the question author or admin can rollback');
  }

  // Perform rollback
  await rollbackQuestion(id, version, session.user.id);
